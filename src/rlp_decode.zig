...
pub fn decodeWithdrawals(alloc: std.mem.Allocator, payload: []const u8) ![]const input.Withdrawal {
    ...
    while (rest.len > 0) {
        ...
        var wp = switch (wr.item) { .list => |p| p, .bytes => continue };
...
pub fn decodeSingleTx(allocator: std.mem.Allocator, raw: []const u8) !input.Transaction {
    ...
    if (raw[0] < 0x80) {
        ...
        const payload = switch (r.item) { .list => |p| p, .bytes => return error.InvalidBlock };
...
fn txAccessList(alloc: std.mem.Allocator, rest: *[]const u8) ![]const input.AccessListEntry {
    const r = mpt.rlp.decodeItem(rest.*) catch return error.InvalidBlock;
    const al_payload = switch (r.item) { .list => |p| p, .bytes => return error.InvalidBlock };
    rest.* = rest.*[r.consumed..];
    ...
    while (al.len > 0) {
        ...
        var ep = switch (er.item) { .list => |p| p, .bytes => return error.InvalidBlock };
        al = al[er.consumed..];

        const addr_r = mpt.rlp.decodeItem(ep) catch return error.InvalidBlock;
        const addr_b = switch (addr_r.item) { .bytes => |b| b, .list => return error.InvalidBlock };
        ...
        const kp_payload = switch (keys_r.item) { .list => |p| p, .bytes => return error.InvalidBlock };
        var keys = std.ArrayListUnmanaged(primitives.Hash){};
        var kp = kp_payload;
        while (kp.len > 0) {
            const k_r = mpt.rlp.decodeItem(kp) catch return error.InvalidBlock;
            const k_b = switch (k_r.item) { .bytes => |b| b, .list => return error.InvalidBlock };
...
fn txHashList(alloc: std.mem.Allocator, rest: *[]const u8) ![]const primitives.Hash {
    ...
    while (p.len > 0) {
        ...
        const b = switch (hr.item) { .bytes => |bs| bs, .list => return error.InvalidBlock };
        if (b.len != 32) return error.InvalidBlock;
...
fn txAuthList(alloc: std.mem.Allocator, rest: *[]const u8) ![]const input.AuthorizationTuple {
    ...
    while (p.len > 0) {
        ...
        var ep = switch (ir.item) { .list => |lp| lp, .bytes => return error.InvalidBlock };
        p = p[ir.consumed..];
